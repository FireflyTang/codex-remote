...
func (c *connection) handleRaw(typ websocket.MessageType, raw []byte, helloOnly bool) bool {
	...
	if helloOnly {
		...
		if h.ClientId == "" || h.ClientRunId == "" || h.ProtocolVersion == nil || h.ProtocolVersion.Major != 1 || h.ProtocolVersion.Minor != 1 || h.ProtocolVersion.Patch != 2 {
			c.protocolClose(remotev1.CloseCode_CLOSE_CODE_PROTOCOL_VERSION_UNSUPPORTED, "protocol 1.1.2 and client identity are required", websocket.StatusProtocolError)
			return false
		}
...
func (c *connection) sendHello() {
	...
	if c.server.cfg.Hello != nil {
		if provided, err := c.server.cfg.Hello(c.ctx); err == nil && provided != nil {
			...
			hello.ProtocolVersion = &remotev1.ProtocolVersion{Major: 1, Minor: 1, Patch: 2}
