...
Current evidence (2026-08-17): the complete self-contained script passes normal, interrupt, approval, user-input, unmanaged sessions, structured items, failed turn, rewatch, synthetic plan/diff, pre-response large updates, large-output, multi-large collection, replay-burst, audit-failure, runtime-disconnect, completed-session restart, and pending-request restart scenarios. After adding `Event.completeness`, the synthetic and early-large scenarios also pass three consecutive combined runs.

| Area | Exercised behavior | Current result |
|---|---|---|
| Upgrade/Hello | correct, missing, and wrong subprotocol; wrong path; exact 1.0; unsupported minor; request before Hello; Host/HostRun/connection identities | pass |
...
| 13 RPCs | every request variant is constructible; GetHost plus directory/session/Codex/create/import/watch/unwatch/history/start/interrupt/approval/input have real scenario coverage | pass |
...
| Turn/items | all 8 Event variants; structured user/reasoning/plan/command/tool/file-change/agent items; command stderr and reasoning/file deltas; failed/interrupted/completed status; Unix-ms times, failure, provenance, complete/incomplete history | pass |
...
| Side-effect dedup | CreateCodex, ImportSession, StartTurn, InterruptTurn, RespondApproval, RespondUserInput: same-payload replay and changed-payload conflict | pass: all 6 |
...
| Restart | external fake JSON persistence; stable Host/Codex/history/dedup; new HostRunID; old paired cursor RESET; unrecoverable pending is cleared, disclosed incomplete, and cannot be falsely answered | pass |
