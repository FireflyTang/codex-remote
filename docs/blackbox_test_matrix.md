...
Current evidence (2026-08-24): one complete self-contained `bash scripts/blackbox.sh` run exited 0 after workspace integration. It passed normal, interrupt, approval, user-input, unmanaged sessions, management lifecycle, workspace, structured items, failed turn, rewatch, synthetic plan/diff, pre-response large updates, large-output, multi-large collection, replay-burst, audit-failure, runtime-disconnect, completed-session restart, lifecycle restart, and pending-request restart scenarios. The black-box scenarios use disposable loopback test Hosts. Separately, the live Tailnet Host was restarted with a build from commit `5f6aee2`; `/healthz` returned 200 and a real wire smoke confirmed protocol 1.1, workspace limits `524288/2097152/2097152/33554432/1000`, and GetWorkspace generation 1 in the allowed state.
...
| 20 Host RPCs | every Host-supported request variant is constructible; the control/lifecycle baseline plus GetWorkspace, ListWorkspaceEntries, ReadWorkspaceTextFile, WriteWorkspaceTextFile, UploadWorkspaceEntry, and DownloadWorkspaceEntry have formal-wire scenarios | pass |
...
| Side-effect dedup | CreateCodex, ImportSession, StartTurn, InterruptTurn, RespondApproval, RespondUserInput, UnmanageCodex, WriteWorkspaceTextFile, UploadWorkspaceEntry: same-payload replay and changed-payload conflict | pass: all 9 |
...
| Workspace V1.1 | six workspace RPCs; five nonzero advertised/enforced limits; paged listing; complete UTF-8 read; atomic create/replace/upsert; strong revision and quiescence preconditions; regular-file/ZIP upload/download; typed path/archive/size errors; generation consistency across Get/RESET/Event; parent-agent busy rejection; read-only access while busy; mutation after the parent stops; unmanaged access; write/upload dedup | pass |
